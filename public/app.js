const landingView = document.getElementById("landingView");
    ...
    (function adminApp() {
      ...
      const els = {
        ...
        deletePackageBtn: document.getElementById("admin-deletePackageBtn"),
        deletePackageStatus: document.getElementById("admin-deletePackageStatus"),
        deleteAllPackagesBtn: document.getElementById("admin-deleteAllPackagesBtn"),
        deleteAllPackagesStatus: document.getElementById("admin-deleteAllPackagesStatus"),
    ...
    async function deleteLatestPackage() {
      ...
      els.deletePackageStatus.textContent = "Deleting...";
      try {
        const data = await apiRequestJson("/api/packages/latest", { method: "DELETE" });
        els.deletePackageStatus.textContent = `Deleted ${data.packageId}`;
        state.packageId = "";
      } catch (err) {
        els.deletePackageStatus.textContent = err.message || "Delete failed";
      }
    }

    async function deleteAllPackages() {
      if (!confirm("Delete ALL published packages? This cannot be undone.")) return;
      els.deleteAllPackagesStatus.textContent = "Deleting...";
      try {
        const data = await apiRequestJson("/api/packages", { method: "DELETE" });
        els.deleteAllPackagesStatus.textContent = `Deleted ${data.count || 0} package(s)`;
        state.packageId = "";
      } catch (err) {
        els.deleteAllPackagesStatus.textContent = err.message || "Delete failed";
      }
    }
    ...
    els.deletePackageBtn.addEventListener("click", deleteLatestPackage);
    els.deleteAllPackagesBtn.addEventListener("click", deleteAllPackages);
    ...
    (function employeeApp() {
      ...
      const els = {
        ...
        attestationNotes: document.getElementById("emp-attestationNotes"),
      ...
      els.attestBtn.addEventListener("click", async () => {
        ...
        const notes = els.attestationNotes.value.trim();
        if (!notes) {
          els.attestationStatus.textContent = "Please enter notes before submitting.";
          els.attestationStatus.className = "alert";
          return;
        }
        if (notes.length > 256) {
          els.attestationStatus.textContent = "Notes must be 256 characters or fewer.";
          els.attestationStatus.className = "alert";
          return;
        }
